...
public class PlexMovieComparisonConfiguration : IEntityTypeConfiguration<PlexMovieComparison>
{
    public void Configure(EntityTypeBuilder<PlexMovieComparison> builder)
    {
        ...
        builder.HasOne<PlexMovie>().WithMany().HasForeignKey(x => x.RemotePlexMediaId).OnDelete(DeleteBehavior.Cascade);

        builder.HasOne<PlexMovie>().WithMany().HasForeignKey(x => x.OwnedPlexMediaId).OnDelete(DeleteBehavior.Cascade);
    }
}
