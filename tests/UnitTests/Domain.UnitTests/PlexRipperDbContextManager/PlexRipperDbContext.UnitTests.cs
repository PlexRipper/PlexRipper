...
public class PlexRipperDbContextManager_UnitTests : BaseUnitTest<PlexRipperDbContextManager>
{
    ...
    public void ShouldConnectToDatabaseAndCheckToMigrate_WhenDatabaseAlreadyExists()
    {
        ...
        mock.Mock<IFileResultSystem>().Setup(x => x.FileExists(It.IsAny<string>())).Returns(true);
    ...
    public void ShouldCreateDatabase_WhenDatabaseDoesNotExist()
    {
        // Arrange
        mock.Mock<IPathProvider>().SetupGet(x => x.DatabasePath).Returns(() => DatabasePath);
        mock.Mock<IFileResultSystem>().Setup(x => x.FileExists(It.IsAny<string>())).Returns(false);
        ...
        mock.Mock<IFileResultSystem>().Verify(x => x.FileExists(It.IsAny<string>()), Times.Once);
    }
    ...
    public void ShouldBackUpAndResetDatabase_WhenDatabaseCannotConnect()
    {
        ...
        mock.Mock<IFileResultSystem>().Setup(x => x.FileExists(It.IsAny<string>())).Returns(true);
        mock.Mock<IFileResultSystem>().Setup(x => x.Copy(It.IsAny<string>(), It.IsAny<string>())).Returns(Result.Ok());
        ...
        mock.Mock<IFileResultSystem>().Verify(x => x.Copy(It.IsAny<string>(), It.IsAny<string>()), Times.Exactly(3));
