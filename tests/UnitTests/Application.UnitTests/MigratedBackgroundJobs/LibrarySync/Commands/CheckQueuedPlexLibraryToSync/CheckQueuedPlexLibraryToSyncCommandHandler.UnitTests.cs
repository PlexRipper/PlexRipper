    ...
    : BaseUnitTest<CheckQueuedPlexLibraryToSyncCommandHandler>
{
    ...
    public async Task ShouldReturnOk_WhenNoQueuedLibrariesExist()
    {
        ...
        Mock.Mock<IBackgroundJobScheduler>().Verify(x => x.IsQueued(It.IsAny<JobKey>()), Times.Never());
    ...
    [Test]
    public async Task ShouldExecuteJob_WhenQueuedLibraryExists()
    {
        // Arrange
        await SetupDatabase(
            1002,
        ...
        Mock.Mock<IBackgroundJobScheduler>().Setup(x => x.IsQueued(jobKey)).ReturnsAsync(false);
        Mock.Mock<IBackgroundJobScheduler>()
            .Setup(x =>
                x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                    It.IsAny<JobKey>(),
        ...
        // Assert
        result.IsSuccess.ShouldBeTrue();
        Mock.Mock<IBackgroundJobScheduler>().Verify(x => x.IsQueued(jobKey), Times.Once());
        Mock.Mock<IBackgroundJobScheduler>()
            .Verify(
                x =>
                    x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                        It.Is<JobKey>(j => j.Equals(jobKey)),
                        It.IsAny<LibrarySyncJobPayload>(),
                        It.IsAny<CancellationToken>()
                    ),
                Times.Once()
    ...
    public async Task ShouldReturnOk_WhenJobAlreadyExists()
    {
        ...
        var jobKey = LibrarySyncJob.GetJobKey(server.Id, library.Id);
        Mock.Mock<IBackgroundJobScheduler>().Setup(x => x.IsQueued(jobKey)).ReturnsAsync(true);
    ...
    public async Task ShouldScheduleHighestPriorityLibrary_WhenMultipleQueuedLibrariesExist()
    {
        ...
        Mock.Mock<IBackgroundJobScheduler>().Setup(x => x.IsQueued(It.IsAny<JobKey>())).ReturnsAsync(false);
        Mock.Mock<IBackgroundJobScheduler>()
            .Setup(x =>
                x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                    It.IsAny<JobKey>(),
        ...
        Mock.Mock<IBackgroundJobScheduler>()
            .Verify(
                x =>
                    x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                        It.Is<JobKey>(j => j.Equals(expectedJobKey)),
                        It.IsAny<LibrarySyncJobPayload>(),
                        It.IsAny<CancellationToken>()
                    ),
                Times.Once()
    ...
    public async Task ShouldScheduleOneLibraryPerServer_WhenMultipleServersWithQueuedLibraries()
    {
        ...
        Mock.Mock<IBackgroundJobScheduler>()
            .Verify(
                x =>
                    x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                        It.Is<JobKey>(j => j.Equals(jobKey1)),
                        It.IsAny<LibrarySyncJobPayload>(),
                        It.IsAny<CancellationToken>()
                    ),
                Times.Once()
        ...
        Mock.Mock<IBackgroundJobScheduler>()
            .Verify(
                x =>
                    x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                        It.Is<JobKey>(j => j.Equals(jobKey2)),
                        It.IsAny<LibrarySyncJobPayload>(),
                        It.IsAny<CancellationToken>()
                    ),
                Times.Once()
    ...
    public async Task ShouldSkipOfflineServerAndScheduleOnlineServer_WhenMixedServerStatuses()
    {
        ...
        Mock.Mock<IBackgroundJobScheduler>().Setup(x => x.IsQueued(onlineServerJobKey)).ReturnsAsync(false);
        Mock.Mock<IBackgroundJobScheduler>()
            .Setup(x =>
                x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                    It.IsAny<JobKey>(),
        ...
        Mock.Mock<IBackgroundJobScheduler>()
            .Verify(
                x =>
                    x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                        It.Is<JobKey>(j => j.Equals(onlineServerJobKey)),
                        It.IsAny<LibrarySyncJobPayload>(),
                        It.IsAny<CancellationToken>()
                    ),
                Times.Once()
    ...
    public async Task ShouldScheduleOnlineServer_WhenDifferentServerHasProcessingLibrary()
    {
        ...
        Mock.Mock<IBackgroundJobScheduler>().Setup(x => x.IsQueued(expectedJobKey)).ReturnsAsync(false);
        Mock.Mock<IBackgroundJobScheduler>()
            .Setup(x =>
                x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                    It.IsAny<JobKey>(),
    ...
    public async Task ShouldNotClearServerOfflineFlag_WhenSameServerHasProcessingLibrary()
    {
        ...
        // Assert
        result.IsSuccess.ShouldBeTrue();
        var queuedItem = await dbContext.LibrarySyncJobQueues.FirstAsync(
            x => x.Status == LibrarySyncJobStatus.Queued,
            CancellationToken
        );
    ...
    public async Task ShouldScheduleOtherServer_WhenOneServerJobAlreadyExists()
    {
        ...
        Mock.Mock<IBackgroundJobScheduler>().Setup(x => x.IsQueued(existingJobKey)).ReturnsAsync(true);
        Mock.Mock<IBackgroundJobScheduler>().Setup(x => x.IsQueued(expectedJobKey)).ReturnsAsync(false);
        Mock.Mock<IBackgroundJobScheduler>()
            .Setup(x =>
                x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
        ...
        Mock.Mock<IBackgroundJobScheduler>().Verify(x => x.IsQueued(existingJobKey), Times.Once());
        Mock.Mock<IBackgroundJobScheduler>().Verify(x => x.IsQueued(expectedJobKey), Times.Once());
        Mock.Mock<IBackgroundJobScheduler>()
            .Verify(
                x =>
                    x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                        It.Is<JobKey>(j => j.Equals(expectedJobKey)),
                        It.Is<LibrarySyncJobPayload>(payload =>
                            payload.PlexServerId == servers[1].Id && payload.PlexLibraryId == secondServerLibrary.Id
                        ),
                        It.IsAny<CancellationToken>()
                    ),
                Times.Once()
        ...
        Mock.Mock<IBackgroundJobScheduler>()
            .Verify(
                x =>
                    x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                        It.Is<JobKey>(j => j.Equals(existingJobKey)),
                        It.IsAny<LibrarySyncJobPayload>(),
                        It.IsAny<CancellationToken>()
                    ),
                Times.Never()
    ...
    public async Task ShouldKeepOfflineFlagForOfflineServerAndClearOnlineServer()
    {
        ...
        Mock.Mock<IBackgroundJobScheduler>().Setup(x => x.IsQueued(onlineJobKey)).ReturnsAsync(false);
        Mock.Mock<IBackgroundJobScheduler>()
            .Setup(x =>
                x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                    It.IsAny<JobKey>(),
        ...
        Mock.Mock<IBackgroundJobScheduler>().Verify(x => x.IsQueued(onlineJobKey), Times.Once());
        Mock.Mock<IBackgroundJobScheduler>()
            .Verify(
                x =>
                    x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                        It.Is<JobKey>(j => j.Equals(onlineJobKey)),
                        It.Is<LibrarySyncJobPayload>(payload =>
                            payload.PlexServerId == servers[1].Id && payload.PlexLibraryId == onlineServerLibrary.Id
                        ),
                        It.IsAny<CancellationToken>()
                    ),
                Times.Once()
