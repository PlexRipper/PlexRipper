...
public class DirectPlexDownloadClientStopAsyncUnitTests : BaseUnitTest<DirectPlexDownloadClient>
{
    ...
    [Test]
    public async Task ShouldHaveFailedResult_WhenPausedStatusUpdateFails()
    {
        // Arrange
        await SetupDatabase(
            33331,
        ...
        stopResult.Errors.ShouldContain(x => x.Message.Contains("Paused status update failed"));

        Mock.Mock<IDownloadTaskUpdateDispatcher>().Verify();
    }

    [Test]
    public async Task ShouldHaveFailedResult_WhenStopIsCalledAgainAndPausedStatusStillFails()
    {
        // Arrange
        await SetupDatabase(
            33332,
        ...
        await Task.Delay(200, CancellationToken);
        var firstStopResult = await sut.StopAsync();
        var startResult = await startTask;
        var secondStopResult = await sut.StopAsync();

        // Assert
        startResult.IsSuccess.ShouldBeTrue();
        firstStopResult.IsFailed.ShouldBeTrue();
        secondStopResult.IsFailed.ShouldBeTrue();
        secondStopResult.Errors.ShouldContain(x => x.Message.Contains("Paused status update failed"));
