...
public interface ISonarrHttpClientFactory
{
...
public class SonarrHttpClientFactory : ISonarrHttpClientFactory
{
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly IReaparrDbContextFactory _dbContextFactory;

    public SonarrHttpClientFactory(IHttpClientFactory httpClientFactory, IReaparrDbContextFactory dbContextFactory)
    {
    ...
    public async Task<Result<HttpClient>> CreateAsync(Guid integrationId, CancellationToken ct)
    {
        ...
        if (result.Value is null)
            return ResultExtensions.EntityNotFound(nameof(SonarrIntegration), integrationId);
