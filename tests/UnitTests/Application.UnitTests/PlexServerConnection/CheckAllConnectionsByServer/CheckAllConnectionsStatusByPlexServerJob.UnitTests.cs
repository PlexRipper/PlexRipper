...
public class CheckAllConnectionsStatusByPlexServerJobUnitTests
    : BaseUnitTest<CheckAllConnectionsStatusByPlexServerJob>
{
    ...
    {
        // Arrange
        await SetupDatabase(135044, config =>
        {
            config.PlexServerCount = 2;
        });

        var plexServerIds = await IDbContext.PlexServers
            .OrderBy(x => x.Id)
            .Select(x => x.Id)
