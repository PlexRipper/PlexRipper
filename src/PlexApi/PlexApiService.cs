...
public class PlexApiService : IPlexApiService
{
    ...
    public async Task<Result<LibraryMetadata>> GetLibraryMediaMetadata(
        PlexLibrary plexLibrary,
        CancellationToken cancellationToken = default
    )
    {
        ...
        if (!int.TryParse(plexLibrary.Key, out var libraryKeyInt))
            return ResultExtensions.IsInvalidId(nameof(plexLibrary.Key), plexLibrary.Key).LogError();

        var countriesTask = _plexApiWrapper.GetLibraryCountries(
            plexServerConnection,
            authToken,
            libraryKeyInt,
            plexLibrary.Type
        );
        var genresTask = _plexApiWrapper.GetLibraryGenres(
            plexServerConnection,
            authToken,
            libraryKeyInt,
            plexLibrary.Type
        ...
        if (countriesResult.IsSuccess)
        {
            response.Countries =
                countriesResult
                    .Value.Object?.MediaContainer?.Directory?.Select(x => new PlexCountry()
                    {
                        Name = x.Title,
        ...
        if (genresResult.IsSuccess)
        {
            response.Genres =
                genresResult
                    .Value.Object?.MediaContainer?.Directory?.Select(x => new PlexGenre()
                    {
                        Name = x.Title,
                        PlexKey = int.Parse(x.Key),
                    })
        ...
        if (actorsResult.IsSuccess)
        {
            response.Roles =
                actorsResult
                    .Value.Object?.MediaContainer?.Directory?.Select(x => new PlexRole
                    {
                        Name = x.Title,
                        PlexKey = int.Parse(x.Key),
