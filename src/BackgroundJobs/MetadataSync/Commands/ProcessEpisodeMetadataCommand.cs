...
public record ProcessEpisodeMetadataCommand(int ServerId, string ServerName) : ICommand<Result<int>>;

public class ProcessEpisodeMetadataCommandValidator : AbstractValidator<ProcessEpisodeMetadataCommand>
{
    public ProcessEpisodeMetadataCommandValidator()
    {
...
public class ProcessEpisodeMetadataCommandHandler : ICommandHandler<ProcessEpisodeMetadataCommand, Result<int>>
{
    ...
    public ProcessEpisodeMetadataCommandHandler(
        ILogger log,
        ICommandExecutor commandExecutor,
        IReaparrDbContextFactory dbContextFactory
    )
    {
        _log = log.ForContext<ProcessEpisodeMetadataCommandHandler>();
        _commandExecutor = commandExecutor;
        _dbContextFactory = dbContextFactory;
    }

    public async Task<Result<int>> ExecuteAsync(ProcessEpisodeMetadataCommand command, CancellationToken ct)
    {
        ...
        _log.Here()
            .Information(
                "Found {Count} Episode rating keys needing metadata enrichment for server {ServerName} ({ServerId})",
        ...
        foreach (var batch in ratingKeysToProcess.Select(k => k.ToString()).Chunk(BATCH_SIZE))
        {
            ...
            if (result.IsFailed)
            {
                _log.Here()
                    .Warning(
                        "Failed to fetch Episode metadata batch for server {ServerName} ({ServerId}): {Error}",
                        command.ServerName,
                        command.ServerId,
            ...
            _log.Here()
                .Debug(
                    "Batch complete: processed {Processed}, skipped {Skipped} Episode parts for server {ServerName} ({ServerId})",
                    processedCount,
                    skippedCount,
                    command.ServerName,
                    command.ServerId
        ...
        if (skippedCount > 0)
        {
            _log.Here()
                .Warning(
                    "Skipped {SkippedCount} Episode parts due to missing metadata for server {ServerName} ({ServerId})",
                    skippedCount,
                    command.ServerName,
                    command.ServerId
                );
