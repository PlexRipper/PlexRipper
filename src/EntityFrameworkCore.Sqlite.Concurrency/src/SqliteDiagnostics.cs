...
namespace EntityFrameworkCore.Sqlite.Concurrency
{
    public static class SqliteDiagnostics
    {
        public static void ShowConcurrencyStats(DbContext context)
        {
            ...
            try
            {
                ...
                if (
                    ...
                    && int.TryParse(results[5], out var free)
                    && total > 0
                )
                {
        ...
        {
            var chart = new BarChart().Width(60).Label("[green bold underline]Performance Metrics[/]").CenterLabel();

            if (stats.Any())
            {
                chart.AddItem("Avg Changes/Sec", stats.Average(s => s.RecentChanges), Color.Green);
                chart.AddItem("Total Changes", stats.Last().TotalChanges, Color.Blue);
            }
