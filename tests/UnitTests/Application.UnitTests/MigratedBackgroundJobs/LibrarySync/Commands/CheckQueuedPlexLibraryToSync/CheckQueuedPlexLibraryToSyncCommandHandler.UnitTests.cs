    ...
    : BaseUnitTest<CheckQueuedPlexLibraryToSyncCommandHandler>
{
    ...
    public async Task ShouldReturnOk_WhenNoQueuedLibrariesExist()
    {
        ...
        Mock.Mock<IBackgroundJobScheduler>()
            .Verify(x => x.IsQueued(It.IsAny<JobKey>()), Times.Never());
        Mock.Mock<IBackgroundJobScheduler>()
            .Verify(
                x => x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                    It.IsAny<JobKey>(),
    ...
    public async Task ShouldScheduleHighestPriorityLibrary_WhenMultipleQueuedLibrariesExist()
    {
        ...
        Mock.Mock<IBackgroundJobScheduler>()
            .Setup(x => x.IsQueued(It.IsAny<JobKey>()))
            .ReturnsAsync(false);
        Mock.Mock<IBackgroundJobScheduler>()
            .Setup(x => x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                It.IsAny<JobKey>(),
    ...
    public async Task ShouldSkipOfflineServerAndScheduleOnlineServer_WhenMixedServerStatuses()
    {
        ...
        Mock.Mock<IBackgroundJobScheduler>()
            .Setup(x => x.IsQueued(onlineServerJobKey))
            .ReturnsAsync(false);
        Mock.Mock<IBackgroundJobScheduler>()
            .Setup(x => x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                It.IsAny<JobKey>(),
                ...
                x =>
                    x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                        ...
                        It.Is<LibrarySyncJobPayload>(payload =>
                            payload.PlexServerId == servers[1].Id
                            && payload.PlexLibraryId == secondServerLibrary.Id
                        ),
                        It.IsAny<CancellationToken>()
                    ),
                Times.Once()
            ...
            .Verify(
                x =>
                    x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                        ...
                        It.Is<LibrarySyncJobPayload>(payload =>
                            payload.PlexServerId == servers[1].Id
                            && payload.PlexLibraryId == onlineServerLibrary.Id
                        ),
                        It.IsAny<CancellationToken>()
                    ),
                Times.Once()
    ...
    public async Task ShouldNotScheduleNonQueuedItems_WhenMixedStatusesExist()
    {
        ...
        Mock.Mock<IBackgroundJobScheduler>()
            .Setup(x => x.IsQueued(expectedJobKey))
            .ReturnsAsync(false);
        Mock.Mock<IBackgroundJobScheduler>()
            .Setup(x => x.ExecuteJob<LibrarySyncJob, LibrarySyncJobPayload>(
                It.IsAny<JobKey>(),
