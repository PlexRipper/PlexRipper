...
public class CompareTvShowPlexLibraryCommandHandler : ICommandHandler<CompareTvShowPlexLibraryCommand, Result>
{
    ...
    )
    {
        ...
        {
            var showMatches = MatchShow(remoteShow, ownedByTmdb, ownedByImdb, ownedByTvdb, ownedByTitleYear);
        ...
        if (seasonRows.Count > 0)
            await dbContext.BulkInsertAsync(seasonRows, cancellationToken: cancellationToken);

        if (episodeRows.Count > 0)
            await dbContext.BulkInsertAsync(episodeRows, cancellationToken: cancellationToken);
    ...
    private static List<(ShowProjection Owned, PlexMediaComparisonMatchType MatchType)> MatchShow(
